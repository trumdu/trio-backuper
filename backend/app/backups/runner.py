...
from backend.app.backups.mongo import backup_mongodump
from backend.app.backups.postgres import backup_pg_dump
from backend.app.backups.retention import cleanup_job_dir
from backend.app.backups.s3 import backup_bucket
...
async def _do_all(job: Job, run_dir: Path) -> tuple[Path, int, str]:
    ...
    # Output path is directory with multiple archives
    return run_dir, int(sizes), "\n".join(logs)
